 ...
 *
 */
class Elementor_Event_Slider extends Widget_Base {
	...
	public function get_title() {
		return __( 'Event Slider', 'wp-event-manager-sliders' );
	}
	/**	
	 * Get widget icon.
	 *
	...
	protected function _register_controls() {
		$this->start_controls_section(
			'section_shortcode',
			[
				'label' => __( 'Event Slider', 'wp-event-manager-sliders' ),
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'type'        => Controls_Manager::NUMBER,
				'default' => '3000',
				'description' => __('Autoplay Speed in milliseconds.','wp-event-manager-sliders')
	...
	protected function render() {
		...
		echo do_shortcode('[events_slider '.$featured.' '.$event_online.' '.$cancelled.' ' .$limit.' '.$orderby.' '.$order.' '.$navigation.' '.$dots.' '.$infinite.' '.$adaptiveheight.' '.$autoplay.' '.$autoplay_speed.']');
	}
